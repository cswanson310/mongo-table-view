(function(factory) {
    ...
    /**
     * Prints 'val', but padded with spaces to be at least width long just left justify for now.
     */
    ...
    /**
     * The header for documents like {a: {b: 1, c: {d: 1}}}, e: {f: 1, g: 1}} would look something
     * like this:
     * +-------------+-----------+
     * | a           | e         |
     * +-----+-------+-----+-----+
    ...
    function makeHeaderStructure(header) {
        /**
         * The field name with one fewer "."s as the level we're on, truncated. e.g. The level 1
         * prefix of "a.b.c" is "a", the level 2 prefix is "a.b".
         */
    ...
    function processField(fieldWidths, field, obj) {
        ...
        if (isObject(val)) {
        ...
        } else if (val instanceof Array && 0 < val.length &&
                   val.length <= config.EXPANDED_ARRAY_ELEMENTS &&
                   Object.keys(val[0]).join(" |").length < config.MAX_FIELD_WIDTH) {
            // recursive case, it's a sub-doc, recurse with prefix field
            Object.keys(val).forEach(function(key) {
                processField(fieldWidths[field], key, obj[field]);
                fieldWidths[field].isArray = true;
            });
        } else {
            ...
            if (val instanceof Array) {
                ...
                if (val.length === 0) {
                    fieldWidths[field].$maxLength =
                        Math.max(fieldWidths[field].$maxLength, "[ ]".length);
                } else {
                    extractArrayLines(val).forEach(function(elt) {
                        var eltLength = elt.length + "[]".length;
                        fieldWidths[field].$maxLength =
                            Math.max(fieldWidths[field].$maxLength, eltLength);
                    });
                }
            } else {
                ...
                fieldWidths[field].$maxLength =
                    Math.max(fieldWidths[field].$maxLength, fieldLength);
            }
        }
    }
    ...
    function parseFields(docs) {
        ...
        function postProcess(currentDoc, path) {
            Object.keys(currentDoc).forEach(function(field) {
                if (field === "$maxLength") {
                    if (currentDoc.$maxLength !== 0) {
                        // Cap the length at MAX_FIELD_WIDTH, unless the field name is larger than
                        // that.
                    ...
                    if (currentDoc.isArray) {
                        nextPath = path + "[" + field + "]";
                    }
                    postProcess(currentDoc[field], nextPath);
                }
            });
        }
    ...
    /**
     * Get each line to print for an array.
     */
    function extractArrayLines(array, prefixStr) {
        var prefix = prefixStr || "";
        var lines = [];
        for (var i = 0; i < array.length; ++i) {
            var elt = array[i];
            if (isObject(elt) && Object.keys(elt).length !== 0) {
                lines.push(prefix + "{");
                for (var key in elt) {
                    if (elt[key] instanceof Array && elt[key].length > 0) {
                        var newPrefix = "  " + prefix;
                        lines.push(newPrefix + key + ": [");
                        lines = lines.concat(extractArrayLines(elt[key], newPrefix + "  "));
                        lines.push(newPrefix + "],");
                        continue;
                    }
                    var value = tojson(
                        elt[key], "" /* indent */, true /* no newlines */, true /* nocolor */);
                    var str = prefix + "  " + key + ": " + value + ",";
                    if (str.length < config.MAX_FIELD_WIDTH) {
                        lines.push(str);
                    } else {
                        var nLines = Math.ceil(str.length / config.MAX_FIELD_WIDTH);
                        var newPrefix = prefix + "    ";
                        for (var line = 0; line < nLines; line++) {
                            var lineSize = config.MAX_FIELD_WIDTH - newPrefix.length;
                            if (line === 0) {
                                lines.push(str.substr(0, config.MAX_FIELD_WIDTH));
                            } else {
                                var start = ((line - 1) * lineSize) + config.MAX_FIELD_WIDTH;
                                lines.push(newPrefix + str.substr(start, lineSize));
                            }
                        }
                    }
                }
                lines.push(prefix + (i === array.length - 1 ? "}" : "},"));
            } else {
                var line = prefix + tojson(elt, "", true, true);
                if (i < array.length - 1) {
                    line += ",";
                }
                lines.push(line);
    ...
    /**
     * Responsible for printing one "row", corresponding to one document.  Note sometimes one "row"
     * can print multiple lines, if there is an array field present, or if there is a value that is
    ...
    /**
     * Given a map of field names to max widths, the documents to display, and the separator between
    ...
    function printTable(header, docs, opts) {
        ...
        docs.forEach(function(doc, i) {
            printRow(
                header,
                docs[i],
                {useUnicode: useUnicode, rowStyle: i === docs.length - 1 ? "bottom" : "middle"});
        });
    }
