(function (factory) {
     ...
     */
    function printRow(header, doc, numRecurses) {
        ...
        for (var field in header) {
            ...
            row += padPrint(val, header[field]);  // Fills in |'s
        }
        row += "|";  // Final border
        print(row);
        if (recurseAgain) {
            printRow(header, doc, numRecurses + 1);
    ...
    var tabularView = function tabularView() {
        var docs = this.toArray();
        if (docs.length === 0) {
            return this;
        }
        printTable(parseFields(docs), docs);
        return this;
    };
    return tabularView;
});
